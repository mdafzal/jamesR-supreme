...
foreach ($product_categories as $category) :
	...
	<li class="product sub-category <?php if ($woocommerce_loop['loop']%$woocommerce_loop['columns']==0) echo 'last'; if (($woocommerce_loop['loop']-1)%$woocommerce_loop['columns']==0) echo 'first'; ?>">
		...
		<a href="<?php echo get_term_link($category->slug, 'product_cat'); ?>" class="product-image-link">
			<?php do_action('woocommerce_before_subcategory_title', $category); ?>
		</a>
		
		<h3 class="product-title"><?php echo $category->name; ?> <?php if ($category->count>0) : ?><mark class="count">(<?php echo $category->count; ?>)</mark><?php endif; ?></h3>
